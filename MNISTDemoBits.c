...
#define EPOCHS 250
#define NUMBER_OF_TRAINING_EXAMPLES 100000
...
unsigned int X_test_2[NUMBER_OF_TEST_EXAMPLES][LA_CHUNKS];
int y_test_2[NUMBER_OF_TEST_EXAMPLES];
	...
	for (int i = 0; i < NUMBER_OF_TEST_EXAMPLES; i++) {
		...
		for (int j = 0; j < FEATURES; j++) {
			if (atoi(token) == 1) {
				int chunk_nr = j / INT_SIZE;
				int chunk_pos = j % INT_SIZE;
				X_test_2[i][chunk_nr] |= (1 << chunk_pos);
			} else {
				int chunk_nr = (j + FEATURES) / INT_SIZE;
				int chunk_pos = (j + FEATURES) % INT_SIZE;
				X_test_2[i][chunk_nr] |= (1 << chunk_pos);
...
{	
	...
	for (int i = 0; i < EPOCHS; i++) {
		...
		printf("Accuracy Dataset I: %.1f\n", 100*test_accuracy);

		float test_2_accuracy = mc_tm_evaluate(mc_tm, X_test_2, y_test_2, NUMBER_OF_TEST_EXAMPLES);
		printf("Accuracy Dataset II: %.1f\n", 100*test_2_accuracy);
	}
